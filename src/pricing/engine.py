...
class TimeDecay:
    ...
    def baseball_factor(
        ...
        Unlike clock sports, remaining "time" is measured in outs.
        Home team bats last = important structural advantage when trailing.
        """
        # Current progress: outs completed
        ...
        # In baseball, expected runs remaining ~ 0.11 * remaining_outs
        expected_remaining_runs = remaining_frac * total_outs * 0.11
        std_remaining = math.sqrt(expected_remaining_runs) if expected_remaining_runs > 0 else 0.1

        # Home bats last advantage: if home is trailing, they get extra outs
        home_advantage = 0.02 if diff < 0 and is_top else 0.0

        z = diff / std_remaining if std_remaining > 0.1 else (10.0 if diff > 0 else -10.0)
        score_based = 1.0 / (1.0 + math.exp(-z * 1.2)) + home_advantage

        score_weight = 1.0 - remaining_frac ** 0.6
        p_home = prior_p_home * (1.0 - score_weight) + score_based * score_weight
...
class ShockAccumulator:
    ...
    @staticmethod
    def baseball_pitcher_change(team: str, at_sec: float) -> EventShock:
        """Pitcher change: new pitcher might be better or worse. Default = neutral."""
        return EventShock("pitcher_change", team, 1.0, 300.0, at_sec)
